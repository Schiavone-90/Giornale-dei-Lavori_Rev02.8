...
// Salva tutto in localStorage come backup + Supabase
function saveDb(db) {
  try {
    localStorage.setItem("giornale_db", JSON.stringify(db));
  } catch (e) {
...
function loadDb() {
  try {
    const stored = localStorage.getItem("giornale_db");
...
async function syncToSupabase(db) {
  try {
    // Salva tabelle principali
    if (db.ditte?.length) {
      for (const d of db.ditte) await dbUpsert("ditte", d);
    }
    if (db.cantieri?.length) {
      for (const c of db.cantieri) await dbUpsert("cantieri", c);
    }
    if (db.anagraficaOperai?.length) {
      for (const o of db.anagraficaOperai) await dbUpsert("anagraficaOperai", o);
    }
    if (db.anagraficaMezzi?.length) {
      for (const m of db.anagraficaMezzi) await dbUpsert("anagraficaMezzi", m);
    }
    if (db.giornali?.length) {
      for (const g of db.giornali) await dbUpsert("giornali", g);
    }
  } catch (e) {
    console.error("Errore sync Supabase:", e);
  }
}

const INIT_DB = {
  dlPassword:"Dl01",
  firmaDL_img:"",          // base64 firma DL
  firmeDitte:{},           // { dittaId: base64 }
  ditte:[],                // {id,nome,piva,referente,password,cantieriIds:[]}
  cantieri:[],             // {id,oggetto,stazione,rup,dl,impresa,cig,cup,luogo}
  anagraficaOperai:[],     // {id,nome,qualifica,dittaId,dittaNome}
  anagraficaMezzi:[],      // {id,nome,tipo,modello,targa,dittaId,dittaNome}
  giornali:[],             /* {
    id, cantiereId,
    numeroBase: "1",        // numero "radice" (es. "1", "2", "12")
    revisioneN: 0,          // 0 = originale, 1 = rev1, 2 = rev2 …
    displayNum: "1",        // stringa visualizzata: "1", "1.1", "1.2" …
    data, meteo, tempMin, tempMax, lavorazioni,
    sospeso, causaSosp, altroMotivo,
    ripresa, dataRipresa, motivoRipresa, noteRipresa,
    operaiPresenti[], mezziUsati[], sopralluoghi[], photos[],
    riserve[],
    firmaDL: false, firmaImpresa: false,
    firmaImpresaDitta:"", firmaImpresaImg:"",
    bloccato: false         // true quando firmato da entrambi
  } */
};
...
const C = {
  ...
  bord:"var(--color-border-tertiary)",
...
function Fld({ label, children, half }) {
  return <div style={{ marginBottom:10, flex:half?1:"unset" }}>{label&&<label style={{ fontSize:12, color:C.txt2, display:"block", marginBottom:4 }}>{label}</label>}{children}</div>;
}
function Row2({ children }) { return <div style={{ display:"flex", gap:8 }}>{children}</div>; }
...
function Divider() { return <div style={{ height:"0.5px", background:C.bord, margin:"10px 0" }}/>; }
...
export default function App() {
  const [db, setDb] = useState(()=>{
    const d=loadDb()||INIT_DB;
    ["dlPassword","firmaDL_img","firmeDitte","ditte","cantieri","anagraficaOperai","anagraficaMezzi","giornali"].forEach(k=>{ if(d[k]===undefined) d[k]=INIT_DB[k]; });
    return d;
  });
  ...
  const upDb = patch => setDb(prev=>{ const n={...prev,...patch}; saveDb(n); return n; });
  const nav = v => setView(v);

  const isDL  = session?.role==="dl";
  const curDitta = db.ditte.find(d=>d.id===session?.dittaId);
  const roleLabel = isDL?"dl":(curDitta?.nome||"Ditta");

  const cantiere = db.cantieri.find(c=>c.id===cantiereId);
  const targetG  = db.giornali.find(g=>g.id===targetGId);
  const cantiereGiornali = db.giornali.filter(g=>g.cantiereId===cantiereId);

  const visibleCantieri = isDL
    ? db.cantieri
    : db.cantieri.filter(c=>(curDitta?.cantieriIds||[]).includes(c.id));

  const logout = ()=>{ setSession(null); nav("login"); };
  const updateG = patch => upDb({ giornali:db.giornali.map(g=>g.id===targetGId?{...g,...patch}:g) });

  /* ── routing ── */
  if (view==="login")   return <Login db={db} onLogin={s=>{ setSession(s); nav("cantieri"); }}/>;
  if (view==="impostazioni") return <Impostazioni db={db} upDb={upDb} onBack={()=>nav("cantieri")} isDL={isDL} role={roleLabel} onLogout={logout}/>;
  if (view==="anagrafica")   return <Anagrafica   db={db} upDb={upDb} onBack={()=>nav("cantieri")} role={roleLabel} onLogout={logout}/>;
  if (view==="form_cantiere") return <FormCantiere initial={targetG?cantiere:undefined} db={db} onSave={c=>{
    if (targetGId) { upDb({cantieri:db.cantieri.map(x=>x.id===cantiere.id?{...x,...c}:x)}); nav("dashboard"); }
    else { upDb({cantieri:[...db.cantieri,{...c,id:genId()}]}); nav("cantieri"); }
  }} onCancel={()=>nav(targetGId?"dashboard":"cantieri")} role={roleLabel} onLogout={logout} isEdit={!!targetG}/>;

  if (view==="form_giornale") {
    const existingNumeri = cantiereGiornali.filter(g=>g.revisioneN===0).map(g=>g.numeroBase);
    return <FormGiornale
      key={targetGId||"new"}
      initial={targetG}
      db={db} cantiereId={cantiereId}
      existingNumeri={existingNumeri}
      onSave={g=>{
        if (targetGId) { upDb({giornali:db.giornali.map(x=>x.id===targetGId?{...x,...g}:x)}); }
        else            { upDb({giornali:[...db.giornali,{...g,id:genId(),cantiereId}]}); }
        nav("lista_giornali");
      }}
      onCancel={()=>nav("lista_giornali")} role={roleLabel} onLogout={logout}/>;
  }

  if (view==="form_revisione"&&targetG) {
    return <FormGiornale
      key={"rev_"+targetGId}
      initial={null} isRevisione
      originalG={targetG}
      db={db} cantiereId={cantiereId}
      existingNumeri={[]}
      onSave={g=>{
  ...
  if (view==="cantieri") return (
    <PageWrap>
      <TopBar title="Cantieri" role={roleLabel} onLogout={logout} extra={isDL&&<Btn v="ghost" sm onClick={()=>nav("impostazioni")}>⚙ Impostazioni</Btn>}/>
      <Pad>
          ...
          <div key={c.id} onClick={()=>{ setCantiereId(c.id); nav("dashboard"); }} style={{ ...cardS, cursor:"pointer", marginBottom:8 }}>
            <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:12, color:C.txt2, marginTop:2 }}>{c.stazione||"—"}{c.cig?` • ${c.cig}`:""}</div>
                <div style={{ display:"flex", gap:6, marginTop:6, flexWrap:"wrap" }}>
                  {c.luogo&&<Bdg>📍 {c.luogo}</Bdg>}
                  <Bdg c="gray">{db.giornali.filter(g=>g.cantiereId===c.id).length} giornali</Bdg>
                </div>
              </div>
  ...
  if (view==="dashboard"&&cantiere) return (
    <PageWrap>
      ...
      <Pad>
        ...
        {isDL&&<Btn v="ghost" sm onClick={()=>{ setTargetGId(cantiere.id); nav("form_cantiere"); }} style={{ marginBottom:10 }}>✏ Modifica cantiere</Btn>}
  ...
  if (view==="view_giornale"&&targetG) return (
    <ViewGiornale
      g={targetG} cantiere={cantiere} isDL={isDL} db={db} role={roleLabel}
      cantiereGiornali={cantiereGiornali}
      onBack={()=>nav("lista_giornali")}
      onEdit={()=>nav("form_giornale")}
      onNuovaRevisione={()=>nav("form_revisione")}
      onUpdate={updateG}
      onLogout={logout} session={session}
    />
  );
...
function Pad({ children }) { return <div style={{ padding:"14px" }}>{children}</div>; }
...
function Login({ db, onLogin }) {
  ...
  const go = () => {
    ...
    if (pwd===db.dlPassword) { onLogin({role:"dl"}); return; }
    const d=db.ditte.find(x=>x.password===pwd);
  ...
  return (
    <div style={{ fontFamily:"var(--font-sans)", minHeight:"100vh", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ ...cardS, width:"100%", maxWidth:340, padding:28 }}>
        <div style={{ textAlign:"center", marginBottom:24 }}>
          <div style={{ fontSize:40, marginBottom:10 }}>🏗</div>
          <div style={{ fontSize:17, fontWeight:700 }}>Giornale dei Lavori</div>
          <div style={{ fontSize:13, color:C.txt2, marginTop:4 }}>Inserisci la tua password per accedere</div>
        </div>
        <Fld label="Password"><input type="password" value={pwd} onChange={e=>{setPwd(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&go()} style={iF} placeholder="••••••••"/></Fld>
        {err&&<div style={{ fontSize:12, color:C.danger, marginBottom:8 }}>{err}</div>}
        <Btn full onClick={go}>Accedi</Btn>
      </div>
    </div>
  );
}
...
/* ─────────────────────────────────────────────
   IMPOSTAZIONI
───────────────────────────────────────────── */
function Impostazioni({ db, upDb, onBack, isDL, role, onLogout }) {
  ...
  const savePass = () => {
    ...
    upDb({dlPassword:newP}); setNewP(""); setConfP(""); setMsg({ok:true,t:"Password aggiornata."});
  };

  const loadImg = (file, cb) => {
    const r=new FileReader(); r.onload=e=>cb(e.target.result); r.readAsDataURL(file);
  };

  const addDitta = () => {
    if (!nNome.trim()||!nPwd.trim()) return;
    upDb({ditte:[...ditte,{id:genId(),nome:nNome.trim(),piva:nPiva.trim(),referente:nRef.trim(),password:nPwd.trim(),cantieriIds:nCant}]});
    setNNome(""); setNPiva(""); setNRef(""); setNPwd(""); setNCant([]);
  };
  const delDitta = id => upDb({ditte:ditte.filter(d=>d.id!==id)});
  ...
  const saveEdit = () => { upDb({ditte:ditte.map(d=>d.id===editId?{...d,...editD}:d)}); setEditId(null); };
    ...
    <PageWrap>
      ...
      <Pad>
        {tab==="account"&&<Crd>
          ...
          {msg&&<div style={{ fontSize:12, color:msg.ok?C.ok:C.danger, marginBottom:8 }}>{msg.t}</div>}
          <Btn onClick={savePass}>Salva password</Btn>
        </Crd>}

        {tab==="firme"&&<>
          <p style={{ fontSize:13, color:C.txt2, marginTop:0 }}>Le firme vengono apposte automaticamente al momento della firma. Carica un'immagine (JPG, PNG, TIFF, ecc.) una sola volta per ciascun soggetto.</p>
          {/* DL */}
          <Crd>
            <STit>Firma Direzione Lavori</STit>
            <input ref={dlFileRef} type="file" accept="image/*" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]) loadImg(e.target.files[0],img=>upDb({firmaDL_img:img})); }}/>
            {db.firmaDL_img
              ? <><img src={db.firmaDL_img} alt="Firma DL" style={{ maxHeight:70, maxWidth:200, border:`1px solid ${C.bord}`, borderRadius:"var(--border-radius-md)", background:"#f9f9f9", display:"block", marginBottom:8 }}/>
                  <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                    <Bdg c="green">Registrata</Bdg>
                    <button onClick={()=>dlFileRef.current.click()} style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:C.accT }}>Sostituisci</button>
                    <button onClick={()=>upDb({firmaDL_img:""})} style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:C.danger }}>Elimina</button>
                  </div>
                </>
              : <Btn v="sec" onClick={()=>dlFileRef.current.click()}>📎 Carica firma DL (immagine)</Btn>
            }
          </Crd>
          {/* Ditte */}
          {ditte.length===0&&<Emp>Nessuna ditta ancora — aggiungila nella sezione "Ditte e accessi".</Emp>}
          {ditte.map(d=>{
            ...
            return (
              <Crd key={d.id}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                  <DittaAvatar nome={d.nome} size={28}/>
                  <span style={{ fontWeight:600, fontSize:14 }}>{d.nome}</span>
                </div>
                <input ref={ditteFileRefs.current[d.id]} type="file" accept="image/*" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]) loadImg(e.target.files[0],img=>upDb({firmeDitte:{...db.firmeDitte,[d.id]:img}})); }}/>
                {fImg
                  ? <><img src={fImg} alt={`Firma ${d.nome}`} style={{ maxHeight:70, maxWidth:200, border:`1px solid ${C.bord}`, borderRadius:"var(--border-radius-md)", background:"#f9f9f9", display:"block", marginBottom:8 }}/>
                      <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                        <Bdg c="green">Registrata</Bdg>
                        <button onClick={()=>ditteFileRefs.current[d.id].current.click()} style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:C.accT }}>Sostituisci</button>
                        <button onClick={()=>{ const f={...db.firmeDitte}; delete f[d.id]; upDb({firmeDitte:f}); }} style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:C.danger }}>Elimina</button>
                      </div>
                    </>
                  : <Btn v="sec" onClick={()=>ditteFileRefs.current[d.id].current.click()}>📎 Carica firma {d.nome} (immagine)</Btn>
                }
              </Crd>
            );
          })}
          ...
          {ditte.map(d=>(
            <Crd key={d.id}>
              {editId===d.id ? <>
                ...
                <p style={{ fontSize:12, color:C.txt2, margin:"4px 0 8px" }}>Cantieri visibili (puoi aggiungerne di nuovi qui):</p>
                {db.cantieri.length===0&&<p style={{ fontSize:12, color:C.txt3 }}>Nessun cantiere creato.</p>}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:12, color:C.txt3, marginTop:2 }}>🔑 <span style={{ fontFamily:"var(--font-mono)" }}>{d.password}</span></div>
                ...
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  ...
                  <Btn v="danger" sm onClick={()=>delDitta(d.id)}>×</Btn>
                </div>
              </div>}
            </Crd>
          ))}
...
/* ─────────────────────────────────────────────
   ANAGRAFICA
───────────────────────────────────────────── */
function Anagrafica({ db, upDb, onBack, role, onLogout }) {
  const [tab, setTab] = useState("ditte");
  ...
  const [aN, setAN] = useState(""); const [aP, setAP] = useState(""); const [aR, setAR] = useState("");
  const ditte = db.ditte||[];

  const addAD = () => {
    if (!aN.trim()||ditte.find(d=>d.nome===aN.trim())) return;
    upDb({ditte:[...ditte,{id:genId(),nome:aN.trim(),piva:aP.trim(),referente:aR.trim(),password:"",cantieriIds:[]}]});
    setAN(""); setAP(""); setAR("");
  };
  const addOp = () => {
    if (!nNome.trim()) return;
    const d=ditte.find(x=>x.nome===nOpD);
    upDb({anagraficaOperai:[...db.anagraficaOperai,{id:genId(),nome:nNome.trim(),qualifica:nQual,dittaId:d?.id||null,dittaNome:d?.nome||""}]});
    setNNome("");
  };
  const addM = () => {
    if (!mNome.trim()) return;
    const d=ditte.find(x=>x.nome===mDitta);
    upDb({anagraficaMezzi:[...db.anagraficaMezzi,{id:genId(),nome:mNome.trim(),tipo:mTipo,modello:mMod.trim(),targa:mTarga.trim().toUpperCase(),dittaId:d?.id||null,dittaNome:d?.nome||""}]});
  ...
  return (
    <PageWrap>
      ...
      <Tabs items={[["ditte","Ditte"],["operai","Operai"],["mezzi","Mezzi"]]} active={tab} onChange={setTab}/>
      <Pad>
        {tab==="ditte"&&<>
          <Crd><STit>Aggiungi ditta</STit>
            <Fld label="Nome"><input value={aN} onChange={e=>setAN(e.target.value)} style={iF} placeholder="Es. Bianchi Impianti Srl"/></Fld>
            <Row2><Fld label="P. IVA" half><input value={aP} onChange={e=>setAP(e.target.value)} style={iF}/></Fld><Fld label="Referente" half><input value={aR} onChange={e=>setAR(e.target.value)} style={iF}/></Fld></Row2>
            <Btn onClick={addAD}>Aggiungi</Btn>
          </Crd>
          {ditte.length===0&&<Emp>Nessuna ditta ancora.</Emp>}
          {ditte.map(d=>(
            <Crd key={d.id}>
              <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:"flex", gap:6, marginTop:6 }}>
                    <Bdg c="blue">👷 {db.anagraficaOperai.filter(o=>o.dittaId===d.id).length}</Bdg>
                    <Bdg>🚜 {db.anagraficaMezzi.filter(m=>m.dittaId===d.id).length}</Bdg>
                    {d.password&&<Bdg c="green">Accesso attivo</Bdg>}
                  </div>
                </div>
                <Btn v="danger" sm onClick={()=>upDb({ditte:ditte.filter(x=>x.id!==d.id),anagraficaOperai:db.anagraficaOperai.map(o=>o.dittaId===d.id?{...o,dittaId:null,dittaNome:""}:o),anagraficaMezzi:db.anagraficaMezzi.map(m=>m.dittaId===d.id?{...m,dittaId:null,dittaNome:""}:m)})}>×</Btn>
              </div>
            </Crd>
          ))}
        </>}
        {tab==="operai"&&<>
          ...
          {groupByDitta(db.anagraficaOperai,ditte).map(g=><GruppoList key={g.id} g={g} type="op" onDel={id=>upDb({anagraficaOperai:db.anagraficaOperai.filter(x=>x.id!==id)})}/>)}
        </>}
        {tab==="mezzi"&&<>
          ...
          {groupByDitta(db.anagraficaMezzi,ditte).map(g=><GruppoList key={g.id} g={g} type="mezzo" onDel={id=>upDb({anagraficaMezzi:db.anagraficaMezzi.filter(x=>x.id!==id)})}/>)}
        </>}
      </Pad>
    </PageWrap>
  );
      ...
      <Crd style={{ padding:"4px 14px" }}>
        {g.items.map((item,i)=>(
          <div key={item.id} style={{ display:"flex", alignItems:"center", borderBottom:i<g.items.length-1?`0.5px solid ${C.bord}`:"none", padding:"9px 0" }}>
            ...
            <button onClick={()=>onDel(item.id)} style={{ background:"none", border:"none", cursor:"pointer", color:C.danger, fontSize:18 }}>×</button>
          </div>
        ))}
      </Crd>
    </div>
    ...
    <PageWrap>
      ...
      <Pad>
        ...
        <Crd><STit>Codici</STit>
          <Row2>
            <Fld label="CIG" half><input value={f.cig} onChange={e=>s("cig",e.target.value)} style={iF}/></Fld>
            <Fld label="CUP" half><input value={f.cup} onChange={e=>s("cup",e.target.value)} style={iF}/></Fld>
          </Row2>
        </Crd>
...
/* ─────────────────────────────────────────────
   FORM GIORNALE  (nuovo + modifica + revisione)
───────────────────────────────────────────── */
function FormGiornale({ initial, isRevisione, originalG, db, cantiereId, existingNumeri, onSave, onCancel, role, onLogout }) {
  ...
  const save = () => {
    ...
    const displayNum = isRevisione ? `${originalG.numeroBase}.${(existingNumeri.length||0)+1}` : numeroBase.trim();
    onSave({ numeroBase:numeroBase.trim(), revisioneN:isRevisione?(existingNumeri.length||0)+1:0, displayNum, data, meteo, tempMin, tempMax, lavorazioni, sospeso, causaSosp, altroMotivo:altroMot, ripresa, dataRipresa:dataRip, motivoRipresa:motRip, noteRipresa:noteRip, operaiPresenti:operai, mezziUsati:mezzi, sopralluoghi:sopra, photos, riserve:src.riserve||[], firmaDL:false, firmaImpresa:false, firmaImpresaDitta:"", firmaImpresaImg:"", bloccato:false });
  };
  ...
  return (
    <PageWrap>
      <TopBar title={isRevisione?`Nuova revisione di N. ${originalG?.numeroBase}`:isEdit?`Modifica N. ${initial?.displayNum}`:"Nuovo giornale"} role={role} onBack={onCancel} onLogout={onLogout}/>
      <Tabs items={TTABS} active={tab} onChange={setTab}/>
      <Pad>
        {tab==="generale"&&<>
          <Crd>
            <Fld label="Numero giornale">
              <input value={numeroBase} onChange={e=>setNumeroBase(e.target.value)} style={iF} placeholder="Es. 1, 2, 12 …" disabled={isRevisione}/>
              {isRevisione&&<div style={{ fontSize:11, color:C.txt3, marginTop:4 }}>Revisione di N. {originalG?.numeroBase} → displayato come {originalG?.numeroBase}.x</div>}
            </Fld>
          ...
          <Crd><STit>Lavorazioni eseguite</STit>
            <textarea value={lavorazioni} onChange={e=>setLavorazioni(e.target.value)} rows={5} style={{ ...iF, resize:"vertical" }} placeholder="Descrivi le lavorazioni..."/>
          </Crd>
        </>}
          ...
          {mG.map(g=>(
            <div key={g.id} style={{ marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                {g.id!=="__none__"&&<DittaAvatar nome={g.label} size={24}/>}
                <span style={{ fontWeight:600, fontSize:13 }}>{g.label}</span>
                <Bdg c="green">{g.items.filter(m=>mezzi.find(x=>x.id===m.id)).length}/{g.items.length}</Bdg>
              </div>
...
function ViewGiornale({ g, cantiere, isDL, db, role, cantiereGiornali, onBack, onEdit, onNuovaRevisione, onUpdate, onLogout, session }) {
  ...
  const checkLock = (patch) => {
    ...
    if (updated.firmaDL && updated.firmaImpresa && !updated.bloccato) {
      patch.bloccato = true;
    }
    onUpdate(patch);
  };
  ...
  const applyFirmaDL = () => {
    if (!db.firmaDL_img) return alert("Prima registra la firma DL nelle Impostazioni → Firme.");
    checkLock({ firmaDL:true });
  };
  const applyFirmaImpresa = () => {
    if (!ditFirmaImg) return alert("La tua firma non è ancora registrata. Contatta la DL.");
    checkLock({ firmaImpresa:true, firmaImpresaDitta:curDitta?.nome||"", firmaImpresaImg:ditFirmaImg });
  };

  const exportPDF = () => {
    const w=window.open("","_blank");
    w.document.write(buildPDF(g,cantiere,ditte,db.firmaDL_img,db.firmeDitte||{}));
    ...
    <PageWrap>
      <TopBar title={`N. ${g.displayNum} — ${g.data}`} role={role} onBack={onBack} onLogout={onLogout}
        extra={<div style={{ display:"flex", gap:6 }}>
          {canEdit&&<Btn v="ghost" sm onClick={onEdit}>✏ Modifica</Btn>}
          {canRevision&&<Btn v="ghost" sm onClick={onNuovaRevisione}>⊕ Revisione</Btn>}
      ...
      <Pad>
        ...
        {tab==="riepilogo"&&<>
          <div style={{ ...cardS, marginBottom:12, display:"flex", gap:14, alignItems:"center" }}>
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:6, marginTop:6, flexWrap:"wrap" }}>
                {g.revisioneN>0&&<Bdg c="yellow">Revisione {g.revisioneN}</Bdg>}
                {g.sospeso&&<Bdg c="yellow">Sospeso</Bdg>}
          ...
          {bloccato&&<div style={{ background:C.okL, borderRadius:"var(--border-radius-md)", padding:"10px 14px", marginBottom:12, fontSize:13, color:C.ok }}>📌 Questo giornale è <b>bloccato</b> (firmato da entrambi). Per apportare modifiche crea una nuova revisione.</div>}
        ...
        {tab==="riserve"&&<>
          <Crd>
            <STit>Riserve dell'impresa</STit>
        ...
        {tab==="firme"&&<>
          <Crd>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", padding:"12px 0", borderBottom:`0.5px solid ${C.bord}`, gap:10 }}>
              <div style={{ flex:1 }}>
                ...
                {g.firmaDL&&db.firmaDL_img&&<img src={db.firmaDL_img} alt="Firma DL" style={{ maxHeight:45, maxWidth:130, marginTop:6, display:"block", border:`0.5px solid ${C.bord}`, borderRadius:"var(--border-radius-md)" }}/>}
              </div>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", padding:"12px 0", gap:10 }}>
              <div style={{ flex:1 }}>
                ...
                {g.firmaImpresa&&g.firmaImpresaImg&&<img src={g.firmaImpresaImg} alt="Firma impresa" style={{ maxHeight:45, maxWidth:130, marginTop:6, display:"block", border:`0.5px solid ${C.bord}`, borderRadius:"var(--border-radius-md)" }}/>}
              </div>
          ...
          {bloccato&&<>
            <div style={{ background:C.okL, borderRadius:"var(--border-radius-md)", padding:"10px 14px", marginBottom:12, fontSize:13, color:C.ok, textAlign:"center" }}>✅ Firmato da entrambe le parti — documento bloccato</div>
...
function buildPDF(g, c, ditte, firmaDLImg, firmeDitte) {
  ...
  const mH =mG.map(gr=>`<b>🏢 ${gr.label}</b><table>${gr.items.map(m=>`<tr><td>${m.nome}</td><td>${m.tipo}</td><td>${m.targa||""}</td></tr>`).join("")}</table>`).join("")||"<p>Nessuno</p>";
  ...
  const pH=g.photos?.map(p=>`<img src="${p.data}" style="max-width:100%;height:auto;margin:8px 0;border-radius:4px;max-height:300px"/>`).join("")||"";
  const fBox=(lbl,nome,firmato,img)=>`<div><b>${lbl}</b><br><small>${nome||""}</small><div class="fb">${img?`<img src="${img}" style="max-height:55px;max-width:160px"/>`:firmato?"<b>FIRMATO</b>":""}</div></div>`;
...
h1{text-align:center;font-size:16px}h2{text-align:center;font-size:13px;color:#555;margin-bottom:16px}
...
table{width:100%;border-collapse:collapse}td,th{padding:4px 6px;border-bottom:1px solid #eee;text-align:left}td.l{color:#666;width:45%}
.fb img{max-width:100%;max-height:60px}
.photos{display:flex;flex-wrap:wrap;gap:10px;margin-top:10px}
.photos img{flex:0 1 calc(50% - 5px);border-radius:4px}
...
${sec("Condizioni Meteo",`<table><tr><td class="l">Condizioni</td><td>${METEO_ICON[g.meteo]||""} ${g.meteo}</td></tr><tr><td class="l">Temperatura</td><td>${g.tempMin||"—"}° — ${g.tempMax||"—"}°C</td></tr></table>`)}
...
${pH?sec("Foto",`<div style="display:flex;flex-wrap:wrap">${pH}</div>`):""}
${sec("Firme",`<div class="fg">
  <div><b>Direzione Lavori</b><div class="fb">${dlSig}</div><p style="font-size:9px;margin:4px 0 0">${c?.dl||""}</p></div>
  <div><b>Impresa Esecutrice</b><div class="fb">${dittaSig}</div><p style="font-size:9px;margin:4px 0 0">${c?.impresa||""}</p></div>
</div>`)}
</body></html>`;
}